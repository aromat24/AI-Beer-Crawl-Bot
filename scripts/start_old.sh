...
get_available_port() {
    ...
    if ! check_port $primary_port; then
        echo $primary_port
    elif ! check_port $backup_port; then
        echo -e "${YELLOW}⚠️ Primary port $primary_port in use, using backup $backup_port for $service_name${NC}"
        echo $backup_port
    else
        echo -e "${RED}❌ Both primary ($primary_port) and backup ($backup_port) ports in use for $service_name${NC}"
        return 1
    fi
}
...
# 2. Clean up existing services with locked port management
echo -e "${BLUE}🧹 Cleaning up existing services...${NC}"

# Clear specific ports we want to use
...
ACTUAL_FLASK_PORT=$(get_available_port $FLASK_PORT $FLASK_PORT_BACKUP "Flask")
if [ $? -ne 0 ]; then
    echo -e "${RED}❌ Cannot start Flask - no available ports${NC}"
    exit 1
fi

nohup python app.py --port=$ACTUAL_FLASK_PORT > logs/flask.log 2>&1 &
FLASK_PID=$!
echo $FLASK_PID > tmp/flask.pid

wait_for_service "http://localhost:$ACTUAL_FLASK_PORT/health" "Flask App"
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Flask application is running (PID: $FLASK_PID, Port: $ACTUAL_FLASK_PORT)${NC}"
else
...
ACTUAL_ADMIN_PORT=$(get_available_port $ADMIN_PORT $ADMIN_PORT_BACKUP "Admin Dashboard")
if [ $? -ne 0 ]; then
    echo -e "${RED}❌ Cannot start Admin Dashboard - no available ports${NC}"
    exit 1
fi

nohup python admin_web.py --port=$ACTUAL_ADMIN_PORT > logs/admin.log 2>&1 &
ADMIN_PID=$!
echo $ADMIN_PID > tmp/admin.pid

wait_for_service "http://localhost:$ACTUAL_ADMIN_PORT" "Admin Dashboard"
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ Admin Dashboard is running (PID: $ADMIN_PID, Port: $ACTUAL_ADMIN_PORT)${NC}"
else
